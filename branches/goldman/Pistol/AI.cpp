...
vector<location> AI::validMoves(int x, int y)
{
  ...
  for(unsigned int o=0;o<6;o++)
  {
    ...
    if(valid[o] && airstrikes.size() > 0 &&  airstrikes[0].x()==x+offset[o].x && airstrikes[0].y()==y+offset[o].y )
    {
      cout<<"closed for air strike"<<endl;
      valid[o]=false;
      stillValid--;
...
bool AI::runHuman()
{
  ...
  if(turnNum() == 1)
  {
    ...
    for(int i=0;i<6;i++)
    {
      Human::spawn(weapons[5], nearby[i].x, nearby[i].y);
    }
  }
  if(turnNum() == 3)
  {
  ...
  for(unsigned int i=0;i<humans.size();i++)
  {
    if(getAirstrikesReady() >=1)
    {
      cout<<"Dropping Ammo"<<endl;
      humans[0].callAmmoDrop(rand()%15-7,rand()%15-7);
    }
    for(unsigned int c=0;c<crates.size();c++)
    {
      if(crates[c].content() == -1)
      {
        if(distance(crates[c].x(),crates[c].y(),humans[i].x(),humans[i].y()) <=1)
        {
          cout<<"Grabbing Ammo"<<endl;
          humans[i].grab(crates[c]);
        }
      }
    }
    ...
    if(weapons[humans[i].weaponID()].delay() < humans[i].idleTime() && humans[i].ammo() > weapons[humans[i].weaponID()].ammo())
    {
      int target = nearest(zombies,humans[i].x(),humans[i].y());
      if(target != -1 && distance(zombies[target].x(),zombies[target].y(),humans[i].x(),humans[i].y() ) <= weapons[humans[i].weaponID()].range() )
      {
        cout<<"Shooting"<<endl;
        humans[i].attack(zombies[target]);
        return false;
      }
    }
...
//end of my code  
  ...
  for(unsigned int i=0;overdose!=16 && i<walls.size();i++)
  {
    if(walls[i].hp()>625)
    {
      overdose=16;
