...
bool AI::runZombie()
{
  ...
  for(unsigned int z=1;z<zombies.size();z++)
  {
    if(zombies[z].id() > zombies[index].id())
    {
      index = z;
    }
  }
  if(zombies.size()>0)
  {
  cout<<"Highest zombie ID: "<<zombies[index].id()<<endl;
  displayZombie(zombies[index]);
  }
  for(unsigned int z=0;z<zombies.size();z++)
  {
    ...
    else if((index=atLocation(walls,inFrontOfMe))!=-1)
    {
      ...
      if(target != -1)
      {
        heading = turnTo(zombies[z].facing(),getDirection(myLoc,location(humans[target].x(),humans[target].y())));
      }
      if(heading == 0)
      {
...
//      cout<<"I caught one mom!"<<endl;
    ...
    else //if nothing is directly in front of you
    {
      ...
      {
        int heading = getDirection(myLoc,location(0,0));
        if(zombies[z].facing() == heading)
        {
          zombies[z].move();
        }
        else
        {
          zombies[z].turn(turnTo(zombies[z].facing(),heading));
        }
      }
    }
  }
